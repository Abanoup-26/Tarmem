...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.beneficiaryNeed.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.beneficiary-needs.update", [$beneficiaryNeed->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unit') ? 'is-invalid' : '' }}" name="unit_id" id="unit_id" required>
                    @foreach($units as $id => $entry)
                        <option value="{{ $id }}" {{ (old('unit_id') ? old('unit_id') : $beneficiaryNeed->unit->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('unit'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $beneficiaryNeed->description) }}">
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('photos_before') ? 'is-invalid' : '' }}" id="photos_before-dropzone">
                </div>
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('photos_after') ? 'is-invalid' : '' }}" id="photos_after-dropzone">
                </div>
            ...
            <div class="form-group">
                <label class="required" for="beneficiary_id">{{ trans('cruds.beneficiaryNeed.fields.beneficiary') }}</label>
                <select class="form-control select2 {{ $errors->has('beneficiary') ? 'is-invalid' : '' }}" name="beneficiary_id" id="beneficiary_id" required>
                    @foreach($beneficiaries as $id => $entry)
                        <option value="{{ $id }}" {{ (old('beneficiary_id') ? old('beneficiary_id') : $beneficiaryNeed->beneficiary->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
