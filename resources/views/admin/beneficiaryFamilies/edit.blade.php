    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.beneficiary-families.update", [$beneficiaryFamily->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $beneficiaryFamily->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('birth_date') ? 'is-invalid' : '' }}" type="text" name="birth_date" id="birth_date" value="{{ old('birth_date', $beneficiaryFamily->birth_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('identity_number') ? 'is-invalid' : '' }}" type="text" name="identity_number" id="identity_number" value="{{ old('identity_number', $beneficiaryFamily->identity_number) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('qualifications') ? 'is-invalid' : '' }}" type="text" name="qualifications" id="qualifications" value="{{ old('qualifications', $beneficiaryFamily->qualifications) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('marital_status') ? 'is-invalid' : '' }}" name="marital_status" id="marital_status" required>
                    ...
                    @foreach(App\Models\BeneficiaryFamily::MARITAL_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('marital_status', $beneficiaryFamily->marital_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\BeneficiaryFamily::ILLNESS_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('illness_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="illness_status_{{ $key }}" name="illness_status" value="{{ $key }}" {{ old('illness_status', $beneficiaryFamily->illness_status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('illness_type') ? 'is-invalid' : '' }}" name="illness_type_id" id="illness_type_id" required>
                    @foreach($illness_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('illness_type_id') ? old('illness_type_id') : $beneficiaryFamily->illness_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\BeneficiaryFamily::JOB_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('job_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="job_status_{{ $key }}" name="job_status" value="{{ $key }}" {{ old('job_status', $beneficiaryFamily->job_status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('job_sallary') ? 'is-invalid' : '' }}" type="number" name="job_sallary" id="job_sallary" value="{{ old('job_sallary', $beneficiaryFamily->job_sallary) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('familyrelation') ? 'is-invalid' : '' }}" name="familyrelation_id" id="familyrelation_id">
                    @foreach($familyrelations as $id => $entry)
                        <option value="{{ $id }}" {{ (old('familyrelation_id') ? old('familyrelation_id') : $beneficiaryFamily->familyrelation->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
