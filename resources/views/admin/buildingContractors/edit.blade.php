...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.buildingContractor.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.building-contractors.update", [$buildingContractor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('visit_date') ? 'is-invalid' : '' }}" type="text" name="visit_date" id="visit_date" value="{{ old('visit_date', $buildingContractor->visit_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('stages') ? 'is-invalid' : '' }}" name="stages" id="stages" required>
                    ...
                    @foreach(App\Models\BuildingContractor::STAGES_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('stages', $buildingContractor->stages) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quotation_with_materials') ? 'is-invalid' : '' }}" type="number" name="quotation_with_materials" id="quotation_with_materials" value="{{ old('quotation_with_materials', $buildingContractor->quotation_with_materials) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quotation_without_materials') ? 'is-invalid' : '' }}" type="number" name="quotation_without_materials" id="quotation_without_materials" value="{{ old('quotation_without_materials', $buildingContractor->quotation_without_materials) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('contractor') ? 'is-invalid' : '' }}" name="contractor_id" id="contractor_id" required>
                    @foreach($contractors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('contractor_id') ? old('contractor_id') : $buildingContractor->contractor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('building') ? 'is-invalid' : '' }}" name="building_id" id="building_id" required>
                    @foreach($buildings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('building_id') ? old('building_id') : $buildingContractor->building->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
