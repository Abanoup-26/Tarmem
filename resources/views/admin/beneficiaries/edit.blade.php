...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.beneficiaries.update", [$beneficiary->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="identity_photo">{{ trans('cruds.beneficiary.fields.identity_photo') }}</label>
            ...
            <div class="form-group">
                <label class="required" for="qualifications">{{ trans('cruds.beneficiary.fields.qualifications') }}</label>
                <input class="form-control {{ $errors->has('qualifications') ? 'is-invalid' : '' }}" type="text" name="qualifications" id="qualifications" value="{{ old('qualifications', $beneficiary->qualifications) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Beneficiary::JOB_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('job_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="job_status_{{ $key }}" name="job_status" value="{{ $key }}" {{ old('job_status', $beneficiary->job_status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('job_title') ? 'is-invalid' : '' }}" type="text" name="job_title" id="job_title" value="{{ old('job_title', $beneficiary->job_title) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('job_salary') ? 'is-invalid' : '' }}" type="number" name="job_salary" id="job_salary" value="{{ old('job_salary', $beneficiary->job_salary) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Beneficiary::ILLNESS_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('illness_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="illness_status_{{ $key }}" name="illness_status" value="{{ $key }}" {{ old('illness_status', $beneficiary->illness_status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('illness_type') ? 'is-invalid' : '' }}" name="illness_type_id" id="illness_type_id">
                    @foreach($illness_types as $id => $entry)
                        <option value="{{ $id }}" {{ (old('illness_type_id') ? old('illness_type_id') : $beneficiary->illness_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('building') ? 'is-invalid' : '' }}" name="building_id" id="building_id" required>
                    @foreach($buildings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('building_id') ? old('building_id') : $beneficiary->building->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
