        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.beneficiaries.update', [$beneficiary->id]) }}"
                ...
                <div class="row justify-content-center">
                    <div class="form-group col-4">
                        ...
                        <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text"
                            name="name" id="name" value="{{ old('name', $beneficiary->name) }}" required>
                        @if ($errors->has('name'))
                    ...
                    <div class="form-group col-4">
                        ...
                        <input class="form-control date {{ $errors->has('birth_date') ? 'is-invalid' : '' }}" type="text"
                            name="birth_date" id="birth_date" value="{{ old('birth_date', $beneficiary->birth_date) }}"
                            required>
                    ...
                    <div class="form-group col-4">
                        ...
                        <input class="form-control {{ $errors->has('identity_number') ? 'is-invalid' : '' }}"
                            ...
                            value="{{ old('identity_number', $beneficiary->identity_number) }}" required>
                ...
                <div class="row justify-content-center">
                    ...
                    <div class="form-group col-3 m-auto">
                        ...
                        <input class="form-control {{ $errors->has('qualifications') ? 'is-invalid' : '' }}" type="text"
                            ...
                            value="{{ old('qualifications', $beneficiary->qualifications) }}" required>
                        ...
                        @foreach (App\Models\Beneficiary::JOB_STATUS_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('job_status') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="job_status_{{ $key }}"
                                    ...
                                    {{ old('job_status', $beneficiary->job_status) === (string) $key ? 'checked' : '' }}
                                    required>
                    ...
                    <div class="form-group col-3 m-auto">
                        ...
                        <input class="form-control {{ $errors->has('job_title') ? 'is-invalid' : '' }}" type="text"
                            name="job_title" id="job_title" value="{{ old('job_title', $beneficiary->job_title) }}">
                ...
                <div class="row justify-content-center">
                    <div class="form-group col-3">
                        ...
                        <input class="form-control {{ $errors->has('job_salary') ? 'is-invalid' : '' }}" type="number"
                            name="job_salary" id="job_salary" value="{{ old('job_salary', $beneficiary->job_salary) }}"
                            step="0.01">
                ...
                <div class="row justify-content-center">
                        ...
                        @foreach (App\Models\Beneficiary::ILLNESS_STATUS_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('illness_status') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="illness_status_{{ $key }}"
                                    ...
                                    {{ old('illness_status', $beneficiary->illness_status) === (string) $key ? 'checked' : '' }}
                                    required>
                                <label class="form-check-label"
                    ...
                    <div class="form-group col-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('illness_type') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($illness_types as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('illness_type_id') ? old('illness_type_id') : $beneficiary->illness_type->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('building') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($buildings as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('building_id') ? old('building_id') : $beneficiary->building->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group col-3 ">
                        <label for="apartment">{{ trans('cruds.beneficiary.fields.apartment') }}</label>
                        <input class="form-control {{ $errors->has('apartment') ? 'is-invalid' : '' }}" type="text"
                            name="apartment" id="apartment" value="{{ old('apartment', $beneficiary->apartment) }}">
                        @if ($errors->has('apartment'))
                            <div class="invalid-feedback">
                                {{ $errors->first('apartment') }}
                            </div>
                        @endif
                        <span class="help-block">{{ trans('cruds.beneficiary.fields.apartment_helper') }}</span>
                    </div>
